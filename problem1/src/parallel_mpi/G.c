...
int main(int argc, char * argv[], char**envp)
{
    ...
    if (myid == 0)
    {
        long long *rx = malloc(nbkt / 2 * sizeof(long long));
        MPI_Barrier(MPI_COMM_WORLD);

        clock_gettime(CLOCK_REALTIME, &t1);

        for (i = 2; i < number_nodes; i++)
        {
            MPI_Recv(rx, nbkt / 2, MPI_LONG_LONG_INT, MPI_ANY_SOURCE, TAG_RESULT, MPI_COMM_WORLD, &ret);
            for (j = 0; j < nbkt / 2; j++)
                histogram_array[j] += rx[j];
        }

        MPI_Recv(rx, nbkt / 2, MPI_LONG_LONG_INT, 1, TAG_FINAL, MPI_COMM_WORLD, &ret);
        for (j = 0; j < nbkt / 2; j++)
            histogram_array[j + nbkt / 2] = rx[j];
    ...
    else if (myid == 1)
    {
        ...
        MPI_Send(histogram_array, nbkt / 2, MPI_LONG_LONG_INT, 0, TAG_FINAL, MPI_COMM_WORLD);
    ...
    else
    {
        ...
        entry(myid - 2, number_nodes - 2, data_array, nin, histogram_array, nbkt);

        MPI_Send(histogram_array, nbkt / 2, MPI_LONG_LONG_INT, 0, TAG_RESULT, MPI_COMM_WORLD);
        MPI_Send(histogram_array + nbkt / 2, nbkt / 2, MPI_LONG_LONG_INT, 1, TAG_RESULT, MPI_COMM_WORLD);
